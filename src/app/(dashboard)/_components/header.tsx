...
export default function Header() {
    ...
    const visibleSegments = accountIndex !== -1 ? segments.slice(0, accountIndex + 1) : segments;
    ...
    const trail = ["Home", ...visibleSegments.map((seg, i) => formatSegment(seg, i))];
        ...
        <div className="text-gray-400 bg-white p-4 flex gap-2 items-center">
            {trail.map((label, idx) => (
                <div key={idx} className="flex items-center gap-2">
                    {idx > 0 && <span>/</span>}
                    <span className={idx === trail.length - 1 ? "text-blue-600 font-medium capitalize" : "capitalize"}>
                        {label}
                    </span>
                </div>
            ))}
