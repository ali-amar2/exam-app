...
export default function FetchQuestions() {
    ...
    if (questions.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-[31rem] space-y-2">
                        ...
                        {questions.map((q: Question, idx: number) => (
                            <div key={q._id} className="p-4 rounded space-y-2">
                                ...
                                {q.answers.map((a: Answer) => {
                                    ...
                                    return (
                                        <div key={a.key} className={`p-3 rounded text-sm ${isCorrect ? "bg-green-100" : isUser ? "bg-red-100" : "bg-muted"}`}>
                                            {a.answer}
                                        </div>
                                    )
                                })}
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            {/* Answers */}
            <RadioGroup key={currentQuestion._id} value={answers[currentQuestion._id] ?? ""} onValueChange={(value) => handleAnswer(currentQuestion._id, value)}
                className="space-y-3"
            >
