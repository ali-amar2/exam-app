...
export default function ReceiveOtp({ email, onSuccess, onBack }: { email: string; onSuccess: () => void; onBack: () => void }) {
    const [counter, setCounter] = useState(60);
    const { mutate, isPending, data } = useReceiveOtp();

    // Timeout is Presisted
    useEffect(() => {
    ...
    return (
        <div className="flex flex-col gap-8 w-3/4">
            <div className="flex flex-col gap-2 ">
                <div className="border border-gray-400 w-fit p-1 mb-10 cursor-pointer " onClick={onBack}>
                    <MoveLeft />
                </div>
                <h1 className="font-inter font-bold text-3xl">Verify OTP</h1>
                <p className="text-gray-500">
                    Please enter the 6-digits code we have sent to: {email}
                    <span onClick={onBack} className="text-blue-600 font-medium cursor-pointer ml-3">Edit</span>
                </p>
            </div>
